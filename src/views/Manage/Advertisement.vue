<template>
  <div class="page-container">
      ...
      <el-form :inline="true" :model="filters" :size="size">
        ...
        <el-form-item>
          <kt-button
            icon="fa fa-plus"
            :label="$t('action.add')"
            perms="manage:advertisement:add"
            type="primary"
            @click="handleAdd"
          />
        </el-form-item>
    ...
    <kt-table
      :height="350"
      permsEdit="manage:advertisement:edit"
      permsDelete="manage:advertisement:delete"
      :data="pageResult"
      :columns="columns"
      @findPage="findPage"
    ...
    >
      ...
      >
        ...
        <el-form-item label="广告名称" prop="advertisementName">
          <el-input v-model="dataForm.advertisementName" auto-complete="off"></el-input>
        </el-form-item>
<!-- 
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { prop: "advertisementName", label: "广告名称", minWidth: 80 },
        { prop: "path", label: "跳转路径", minWidth: 80 },
        { prop: "pictureUrl", label: "图片路径", minWidth: 80 },
  ...
  methods: {
	...
	cos.putObject({
		...
		// console.log(err || data);
		 if(data){
			 that.dataForm.pictureUrl='https://'+data.Location
		 }
    ...
    findPage: function(data) {
      ...
      this.$api.advertisement.findPage(this.pageRequest).then(res => {
      ...
      this.$refs.dataForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            this.$api.advertisement.save(params).then(res => {
              if (res.code == 200) {
