  ...
  <div>
    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column :label="$t('action.operation')" width="185" fixed="right" v-if="showDetail" header-align="center" align="center">
        <template slot-scope="scope">
          <kt-button icon="fa fa-edit" :label="$t('action.detail')" :perms="permsEdit" :size="size" @click="getSingleDetail(scope.$index, scope.row)" />
          <kt-button icon="fa fa-edit" :label="$t('action.handle')" :perms="permsEdit" :size="size" @click="handleOrder(scope.$index, scope.row)" />
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
...
export default {
  ...
  methods: {
    ...
    // 编辑
     ...
     //查看单条记录详情
  getSingleDetail:function(index,row){
   this.$emit('getSingleDetail', {index:index, row:row})
